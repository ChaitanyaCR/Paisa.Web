...
describe('inMonth', () => {
  it('keeps only the selected month', () => {
    expect(inMonth(transactions, '2026-09').map((t) => t.id)).toStrictEqual([
      '1',
      '2',
      '3',
      '4',
...
describe('filterTransactions', () => {
  it('returns the month newest first', () => {
    expect(
      filterTransactions(transactions, categories, base).map((t) => t.id),
...
describe('buildBreakdown', () => {
  ...
  it('ranks expense categories by total and omits empty ones', () => {
    expect(
      buildBreakdown(categories, month).map((c) => [c.id, c.total]),
    ).toStrictEqual([
      ['home', 1800000],
      ['food', 330000],
    ]);
  ...
  it('excludes income categories entirely', () => {
    expect(
      buildBreakdown(categories, month).some((c) => c.id === 'salary'),
    ).toBe(false);
  });
});
...
describe('buildCategoryReport', () => {
  it('includes income and expense, with entry counts', () => {
    const month = inMonth(transactions, '2026-09');
    expect(
      buildCategoryReport(categories, month).map((c) => [
        c.id,
        c.total,
        c.count,
      ]),
...
describe('buildChartData', () => {
  ...
  it('produces four bars across a month', () => {
    ...
    expect(
      bars.reduce((sum, b) => sum + b.income + b.expense, 0),
    ).toBeGreaterThan(0);
  });
  ...
  it('keeps an entered calendar date in its intended bucket', () => {
    const bars = buildChartData(
      [
        {
          id: 'boundary',
    ...
    expect(bars.map((bar) => bar.expense)).toEqual([0, 100]);
  });
});
...
describe('chartCeiling', () => {
  ...
  it('otherwise uses the largest bar', () => {
    expect(
      chartCeiling([{ label: 'a', income: 5000000, expense: 120000 }]),
    ).toBe(5000000);
  });
});
